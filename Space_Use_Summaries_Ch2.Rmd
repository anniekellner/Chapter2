...
It is noteworthy that all females with COYS visit the bonepile. This makes sense, as COYs are vulnerable to starvation and infanticide is not likely to be a threat when food is abundant. Interesting to conjecture that the demographic benefit of the bonepile could possibly be due to enhanced COY survival. Seems like it would be possible to look at this. Hmmm. Idea for chapter 3. 

If yearlings and COYs are combined, is there an association with 'dependent young' or 'has coy' and the bonepile?
...
There is not an association between having dependent young and the bonepile. How about having a COY?

```{r}

r <- r %>%
  dplyr::mutate(coy = case_when( 
    repro == "coy" ~ "yes",
    TRUE ~ "no"
  ))

fisher.test(r$bear_type, r$coy)
